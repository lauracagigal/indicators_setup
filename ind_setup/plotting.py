...
def plot_trendline_year(data, var, ax, color = 'k'):
    ...
    _, _, _, p_value, _ = linregress(time_num, data.values)
    if p_value < 0.05:
        ax.plot(time_num, trendline(time_num), color=color, linestyle='-', label='Trendline (p < 0.05 S.Signigicant)')
    else:
        ax.plot(time_num, trendline(time_num), color=color, linestyle=':', label='Trendline (p > 0.05 Not Signigicant)')
...
def plot_bar_probs(x, y, bar_label = None, labels = None, trendline = False, 
    ...
    if trendline:
        ...
        if p_value < 0.05:
            ax.plot(time_num, trendline(time_num), color='k', linestyle='-', label='Trendline (p < 0.05 S.Signigicant)')
        else:
            ax.plot(time_num, trendline(time_num), color='k', linestyle=':', label='Trendline (p > 0.05 Not Signigicant)')
...
def plot_map_subplots(data_an, var, shp_eez = None, cmap='RdBu_r', vmin=-.3, vmax=.3, 
                      sub_plot = None, figsize = (17, 20), cbar = None, cbar_pad = -0.02):
    ...
    if cbar is not None:
        fig.colorbar(im, ax=axs, label=var, orientation='horizontal', shrink=0.6, 
                     pad = cbar_pad).set_label(var, fontsize = fontsize)
    plt.show()
...
def plot_bar_probs_ONI(df2, var, y_label = ''):
    ...
    cbar.set_label('ONI Categories', fontsize=fontsize)
